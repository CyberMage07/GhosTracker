...
def phoneGW():
    ...
    # New feature: Getting latitude and longitude for the location
    lat_lon_req = requests.get(f"https://nominatim.openstreetmap.org/search?q={location}&format=json")
    link2 = "https://www.google.com/maps/place/Alliance+Nisarg,+Shankar+Kalat+Nagar,+Wakad,+Pimpri-Chinchwad,+Maharashtra+411057/@18.6064287,73.75828,19z/data=!4m6!3m5!1s0x3bc2b97a0dc194fd:0x2eb4fa9246059da2!8m2!3d18.6064656!4d73.7588674!16s%2Fg%2F11h08tqcs?entry=ttu"
    lat_lon_data = lat_lon_req.json()
    
    if lat_lon_data:
        latitude = lat_lon_data[0]["lat"]
        longitude = lat_lon_data[0]["lon"]
        maps_link = f"https://www.google.com/maps/@{latitude},{longitude},15z"
    else:
    ...
    # Display latitude, longitude, and maps link
    print(f"\n {Wh}========== {Gr}LOCATION INFORMATION {Wh}==========")
    print(f" {Wh}Latitude             :{Gr} {latitude}")
    print(f" {Wh}Longitude            :{Gr} {longitude}")
    print(f" {Wh}Google Maps Link     :{Gr} {maps_link}")
    print(f"Location : {link2}")
